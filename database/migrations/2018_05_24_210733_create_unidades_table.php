...
class CreateUnidadesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('unidades', function (Blueprint $table) {
            ...
            $table->decimal('area_util',10,3)->nullable();
            $table->decimal('area_total',10,3)->nullable();
            $table->decimal('indice',10,6);
            $table->enum('endereco_principal', ['Condominio','Proprietario','Inquilino']);
            $table->enum('boleto_impresso', ['Sim', 'Nao'])->default('Nao');
            $table->enum('boleto_impresso_destino', ['Portaria', 'Correios'])->nullable();
            $table->enum('boleto_email', ['Sim', 'Nao'])->default('Nao');
            $table->enum('boleto_email_destino', ['Proprietario', 'Inquilino','Todos'])->nullable();
